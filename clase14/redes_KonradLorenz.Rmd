...
library(nnet)
set.seed(12345)
mod_1 = nnet(Species ~ ., datos_entrenamiento, size = 3, maxit = 1000)
```
...
```{r}
prediction = predict(mod_1, datos_prueba, type = "raw")
rowSums(prediction)
```
...
library(nnet)
set.seed(12345)
mod_1b = multinom(Species ~ ., datos_entrenamiento, maxit = 10000)
prediction2 = predict(mod_1b, datos_prueba, type = "class")
real_class2 = datos_prueba$Species
cat("Matriz de Confusión: \n")

table(prediction2, real_class2)
...
colnames(datos_prueba) = c("cemento", "escoria", "ceniza", "agua", 
...
concreto1 = as.data.frame(lapply(datos_entrenamiento, minmax))
concreto2 <- data.frame(datos_entrenamiento$`Concrete compressive strength(MPa, megapascals)`)
Datos_entrenamiento <- cbind(concreto1, concreto2) 
colnames(Datos_entrenamiento) = c("cemento", "escoria", "ceniza", "agua", 
                       "plastif", "grueso", "fino", "edad", "fuerza")
```


```{r}
min_train <- sapply(datos_entrenamiento, min)
max_train <- sapply(datos_entrenamiento, max)
```


```{r}
for(j in 1:9){
  datos_prueba[,j] <- (datos_prueba[,j] - min_train[j]) / (max_train[j] - min_train[j])
}
```
...
resultados = compute(mod_2, datos_prueba)
fuerza_pred = resultados$net.result
fuerza_pred = fuerza_pred * (max_train[9] - min_train[9]) +  min_train[9]
datos_prueba$fuerza = datos_prueba$fuerza * (max_train[9] - min_train[9]) +  min_train[9]

cat("La correlación entre la predicción y los datos reales es: ")
cor(fuerza, datos_prueba$fuerza)
plot(fuerza, fuerza_pred)
abline(a = 0, b = 1, col = "red")
sqrt(mean((fuerza_pred - datos_prueba$fuerza)^2))
```
...
```{r}
h2o.performance(mod_capas, test.h2o)
