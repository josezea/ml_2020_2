...
set.seed(26092020)
particion <- initial_split(insumo, prop = 0.7)
class(particion)
train_cel <- particion %>% training() #  training(particion);  f(x, y)  <=>  x %>% f(y)
test_cel <- particion %>% testing()
# set.seed(17092020)
...
plot(1-especificidad, sensibilidad, xlim = c(0,1), ylim = c(0,1), pch = 20, 
     xlab = "Error Tipo 1", ylab  = "Sensibilidad", main = "ROC")
abline(a = 0, b = 1, col = "red")
# Estimación del punto de la curva ROC asociado a p = 0.5

# Ejercico 2: calcular la curva ROC
probs_corte <- seq(0.05, 0.95, 0.05)
vctr_sensibilidad <- rep(NA, length(probs_corte))
vctr_especificicad <- rep(NA, length(probs_corte))

probs_churn <- predict(M1, test_cel, type = "response")
for(i in 1:length(probs_corte)){

mc_churn <- table(test_cel$target, 
                  as.numeric(probs_churn >= probs_corte[i]))
...
names(vctr_sensibilidad) <- probs_corte
names(vctr_especificicad) <- probs_corte

plot(1-vctr_especificicad, vctr_sensibilidad, pch = 20 , xlab = "1- Espec (Error tipo 2)", 
     ylab = "Sensibilidad", xlim = c(0,1), ylim = c(0,1), main = "ROC")
lines(1-vctr_especificicad, vctr_sensibilidad, col = "blue" )
text(1-vctr_especificicad - 0.03, vctr_sensibilidad, labels = probs_corte, cex = 0.7)
abline(a = 0, b = 1, col = "red")
abline(v = 1 - vctr_especificicad[names(vctr_especificicad) == "0.5"], col = "green")
abline(h = vctr_sensibilidad[names(vctr_sensibilidad) == "0.5"], col = "green")
